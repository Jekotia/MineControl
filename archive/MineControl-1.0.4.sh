#!/bin/bash
minecontrol_Version="1.0.4" # By Jekotia https://github.com/Jekotia/MineControl
	...
	if isrunning; then
		...
		if [ "$log_roll_worldedit_Enable" = "true" ]; then
			if [ ! -f "${server_Dir}worldedit.log" ]; then
				...
				mv -n ${server_Dir}worldedit.log ${log_Dir}worldedit/$(date '+%Y-%m-%d')_$(date '+%H-%M-%S').worldedit.log && echo "${server_Dir}worldedit.log moved to ${log_Dir}worldedit/$(date '+%Y-%m-%d')_$(date '+%H-%M-%S').log" || echo "Failed to move ${server_Dir}worldedit.log to ${log_Dir}worldedit/$(date '+%Y-%m-%d')_$(date '+%H-%M-%S').log"
			fi
		fi
	fi
}
