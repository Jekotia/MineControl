...
# WARNING! ONLY CHANGE THE BELOW VARIABLES IF YOU ARE VERY SURE OF WHAT YOU ARE DOING
	...
	log_Minecontrol="${minecontrol_Dir}minecontrol.log"
...
_common_log() {
	if [ "$log_scriptEvents" = "true" ]; then
		if [ "$log_scriptEvents_append" = "true" ]; then
			echo $(date '+%Y-%m-%d')_$(date '+%H-%M-%S')": $1" >> ${log_Minecontrol}
		else
			echo $(date '+%Y-%m-%d')_$(date '+%H-%M-%S')": $1" | cat - ${log_Minecontrol} > ${temp_Dir}minecontrol.log.temp && mv ${temp_Dir}minecontrol.log.temp ${log_Minecontrol}
		fi
	fi
}
